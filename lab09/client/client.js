...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        const userList = qs("#users-list");
        userList.innerHTML = '';
        data.active_users.forEach(user => {
            let newLI = document.createElement("li")
            let currList = qs("#users-list");
        ...
        const chatList = qs("#chat");
        // append to the chatList
        const chat = document.createElement("p");
        chat.innerHTML = `<b>${data.username}:</b> ${data.text}`;
        // chat.innerText = `${data.username}: ${data.text}`;
